...
def read_contents(dir: pathlib.Path) -> typing.List[str]:
        ...
        for elem in files:
                ...
                for tag in soup.find_all("div", "text"):
                        ...
                        if len(element) >= 35 and len(element) <=100 and not "@" in element and not "//" in element and element[1:element.index("\n", 3)+1] not in things:
                                things.append(element[1:element.index("\n", 3)+1])
        return sorted(things)


def write_contents(contents: typing.List[str], path: pathlib.Path = pathlib.Path("test.txt")) -> None:
        option = "w"
